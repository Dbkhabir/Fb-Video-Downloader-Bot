...
async def get_video_info(url: str):
    # லிங்கেকে API এর বোঝার উপযোগী (Encode) করা হচ্ছে
    encoded_url = urllib.parse.quote(url, safe='')
    api_url = f"https://zylalabs.com/api/2013/facebook+video+downloader+api/1888/get+video?url={encoded_url}"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url, headers=headers) as response:
                ...
                if response.status != 200:
                    ...
                    logger.error(f"ZylaLabs Error: {error_msg}")
                    return None
                    
                data = await response.json()
                logger.info(f"ZylaLabs Response Data: {data}") # Railway লগে ডেটা দেখাবে
                return data
...
async def handle_fb_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not video_data:
        await msg.edit_text("❌ API Error! Check Railway Logs for details.")
        return

    # Extract Links securely
    sd_link = video_data.get('sd_url') or video_data.get('sd') or video_data.get('url')
    hd_link = video_data.get('hd_url') or video_data.get('hd')

    if not sd_link and not hd_link:
        await msg.edit_text("❌ Video not found or link is private. Check Railway logs.")
        return

    vid_id = str(uuid.uuid4())[:8]
    
    hd_size = await get_file_size(hd_link) if hd_link else 0
    sd_size = await get_file_size(sd_link) if sd_link else 0
